...
func (d *directory) writeContents(contents []byte, record wadlib.ContentRecord, byId bool) error {
	// We default to reading by the index.
	name := ""
	if byId {
		name = fmt.Sprintf("%08x.app", record.ID)
	} else {
		name = fmt.Sprintf("%08x.app", record.Index)
	}
